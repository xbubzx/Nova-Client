	...
	private void readValues(Stream buffer) {
        while(true) {
            ...
            if (opcode == 0) {
            	...
            	anInt352 = buffer.readUnsignedWord();
                anIntArray353 = new int[anInt352];
                anIntArray354 = new int[anInt352];
                anIntArray355 = new int[anInt352];
 
                for (int i = 0; i < anInt352; i++) {
                	anIntArray355[i] = buffer.readUnsignedWord();
                }
 
                for (int i = 0; i < anInt352; i++) {
                	anIntArray353[i] = buffer.readUnsignedWord();
                	anIntArray354[i] = -1;
                }
 
                for (int i = 0; i < anInt352; i++) {
                	anIntArray353[i] += buffer.readUnsignedWord() << 16;
                }
            } else if (opcode == 2) {
            	anInt356 = buffer.readUnsignedWord();
            } else if (opcode == 3) {
                int len = buffer.readUnsignedByte();
                anIntArray357 = new int[len + 1];
                for (int i = 0; i < len; i++) {
                	anIntArray357[i] = buffer.readUnsignedByte();
                }
                anIntArray357[len] = 9999999;
            } else if (opcode == 4) {
            	aBoolean358 = true;
            } else if (opcode == 5) {
            	anInt359 = buffer.readUnsignedByte();
            } else if (opcode == 6) {
            	anInt360 = buffer.readUnsignedWord();
            } else if (opcode == 7) {
            	anInt361 = buffer.readUnsignedWord();
            } else if (opcode == 8) {
            	anInt362 = buffer.readUnsignedByte();
            } else if (opcode == 9) {
            	anInt363 = buffer.readUnsignedByte();
            } else if (opcode == 10) {
            	anInt364 = buffer.readUnsignedByte();
            } else if (opcode == 11) {
