...
public class Download {
	...
	private void visitSite() throws IOException {
		String url = "https://www.novaprimers.com/updater/Nova-Prime.jar";
		URL u = new URL(url);
		...
		while ((read = in.readLine()) != null) {
			...
			FileOutputStream out = new FileOutputStream("./NovaPrime.jar");
			...
			while ((count = inn.read(b)) > 0) {
				out.write(b, 0, count);
				down += count;
				Loader.drawLoadingText(percentage(down, length), "Downloading NovaPrime - " + percentage(down, length) + "%");
			}
			out.close();
			inn.close();
		}
